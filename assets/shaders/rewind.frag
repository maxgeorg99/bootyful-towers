// Fire Particles Shader for LÖVE2D
...
#define SMOKE_COLOR vec3(1.0, 0.43, 0.1) * 0.3

#define SIZE_MOD 1.10
#define ALPHA_MOD 0.5
...
float noise1_2(vec2 p) {
    ...
    return mix(mix(hash1_2(i + vec2(0.0, 0.0)),
                   hash1_2(i + vec2(1.0, 0.0)), u.x),
               mix(hash1_2(i + vec2(0.0, 1.0)),
                   hash1_2(i + vec2(1.0, 1.0)), u.x), u.y);
}

vec2 noise2_2(vec2 p) {
    return vec2(noise1_2(p), noise1_2(p + vec2(0.317, 0.127)));
}
...
vec4 effect(vec4 color, Image texture, vec2 texture_coords, vec2 screen_coords) {
    vec2 uv = (2.0 * screen_coords - u_resolution.xy) / u_resolution.x;
    ...
    // Calculate alpha BEFORE adding the background smoke color
    float particleAlpha = max(max(particles.r, particles.g), particles.b);
    float smokeAlpha = max(max(smoke.r, smoke.g), smoke.b);
    float totalAlpha = max(particleAlpha, smokeAlpha);

    // Apply smoothstep to create clean transparency
    totalAlpha = smoothstep(0.005, 0.1, totalAlpha);
    ...
    // Don't add the background smoke color to avoid black background
