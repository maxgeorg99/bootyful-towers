...
local function unique(state, arguments, level)
  ...
  if
    type(arguments[2]) == "boolean" or (arguments[2] == nil and argcnt > 2)
  then
    deep = arguments[2]
...
local function near(state, arguments, level)
  ...
  assert(
    argcnt > 2,
    s("assertion.internal.argtolittle", { "near", 3, tostring(argcnt) }),
    level
  )
  ...
  assert(
    ...
    s(
      "assertion.internal.badargtype",
      { 3, "near", numbertype, format(arguments[3]) }
    ),
    level
  )
...
local function matches(state, arguments, level)
  ...
  assert(
    argcnt > 1,
    s("assertion.internal.argtolittle", { "matches", 2, tostring(argcnt) }),
    level
  )
  ...
  for i = 3, argcnt, 1 do
    if
      arguments[i]
      and type(arguments[i]) ~= "boolean"
      and not tonumber(arguments[i])
    then
      if i == 3 then
  ...
  assert(
    ...
    s(
      "assertion.internal.badargtype",
      { 2, "matches", stringtype, format(arguments[2]) }
    ),
    level
  )
  assert(
    ...
    s(
      "assertion.internal.badargtype",
      { init_arg_num, "matches", "number", type(arguments[3]) }
    ),
    level
  )
  -- switch arguments for proper output message
...
local function equals(state, arguments, level)
  ...
  assert(
    argcnt > 1,
    s("assertion.internal.argtolittle", { "equals", 2, tostring(argcnt) }),
    level
  )
...
local function same(state, arguments, level)
  ...
  assert(
    argcnt > 1,
    s("assertion.internal.argtolittle", { "same", 2, tostring(argcnt) }),
    level
  )
...
local function truthy(state, arguments, level)
  ...
  assert(
    argcnt > 0,
    s("assertion.internal.argtolittle", { "truthy", 1, tostring(argcnt) }),
    level
  )
  set_failure_message(state, arguments[2])
...
local function falsy(state, arguments, level)
  ...
  assert(
    argcnt > 0,
    s("assertion.internal.argtolittle", { "falsy", 1, tostring(argcnt) }),
    level
  )
...
local function has_error(state, arguments, level)
  ...
  assert(
    ...
    s(
      "assertion.internal.badargtype",
      { 1, "error", "function or callable object", type(func) }
    ),
    level
  )
...
local function error_matches(state, arguments, level)
  ...
  assert(
    ...
    s(
      ...
      { "error_matches", 2, tostring(argcnt) }
    ),
    level
  )
  assert(
