...
local matcher_mt = {
  __call = function(self, value) return self.callback(value) == self.mod end,
}
...
local match = {
  _ = setmetatable(
    { mod = true, callback = function() return true end },
    matcher_mt
  ),
  ...
  is_ref_matcher = function(object)
    local ismatcher = (
      type(object) == "table" and getmetatable(object) == matcher_mt
    )
