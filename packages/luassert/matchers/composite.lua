...
local function none(state, arguments, level)
  ...
  assert(
    argcnt > 0,
    s("assertion.internal.argtolittle", { "none", 1, tostring(argcnt) }),
    level
  )
  for i = 1, argcnt do
    assert(
      ...
      s(
        ...
        { 1, "none", "matcher", type(arguments[i]) }
      ),
      level
    )
  end
...
local function any(state, arguments, level)
  ...
  assert(
    argcnt > 0,
    s("assertion.internal.argtolittle", { "any", 1, tostring(argcnt) }),
    level
  )
  for i = 1, argcnt do
    assert(
      ...
      s(
        "assertion.internal.badargtype",
        { 1, "any", "matcher", type(arguments[i]) }
      ),
      level
    )
  end
...
local function all(state, arguments, level)
  ...
  assert(
    argcnt > 0,
    s("assertion.internal.argtolittle", { "all", 1, tostring(argcnt) }),
    level
  )
  for i = 1, argcnt do
    assert(
      ...
      s(
        "assertion.internal.badargtype",
        { 1, "all", "matcher", type(arguments[i]) }
      ),
      level
    )
  end
