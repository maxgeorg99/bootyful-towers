...
local function format(val) return astate.format_argument(val) or tostring(val) end
...
local function near(state, arguments, level)
  ...
  assert(
    argcnt > 1,
    s("assertion.internal.argtolittle", { "near", 2, tostring(argcnt) }),
    level
  )
  ...
  assert(
    ...
    s(
      ...
      { 1, "near", numbertype, format(arguments[1]) }
    ),
    level
  )
  assert(
    ...
    s(
      "assertion.internal.badargtype",
      { 2, "near", numbertype, format(arguments[2]) }
    ),
    level
  )
...
local function matches(state, arguments, level)
  ...
  assert(
    argcnt > 0,
    s("assertion.internal.argtolittle", { "matches", 1, tostring(argcnt) }),
    level
  )
  ...
  assert(
    ...
    s(
      "assertion.internal.badargtype",
      { 1, "matches", "string", type(arguments[1]) }
    ),
    level
  )
  assert(
    ...
    s(
      "assertion.internal.badargtype",
      { 2, "matches", "number", type(arguments[2]) }
    ),
    level
  )
...
local function equals(state, arguments, level)
  ...
  assert(
    argcnt > 0,
    s("assertion.internal.argtolittle", { "equals", 1, tostring(argcnt) }),
    level
  )
  return function(value) return value == arguments[1] end
end
...
local function same(state, arguments, level)
  ...
  assert(
    argcnt > 0,
    s("assertion.internal.argtolittle", { "same", 1, tostring(argcnt) }),
    level
  )
  return function(value)
...
local function ref(state, arguments, level)
  ...
  assert(
    argcnt > 0,
    s("assertion.internal.argtolittle", { "ref", 1, tostring(argcnt) }),
    level
  )
  assert(
    isobject,
    s("assertion.internal.badargtype", { 1, "ref", "object", argtype }),
    level
  )
  return function(value) return value == arguments[1] end
end
...
local function is_true(state, arguments, level)
  return function(value) return value == true end
end

local function is_false(state, arguments, level)
  return function(value) return value == false end
end

local function truthy(state, arguments, level)
  return function(value) return value ~= false and value ~= nil end
end

local function falsy(state, arguments, level)
  local is_truthy = truthy(state, arguments, level)
  return function(value) return not is_truthy(value) end
end

local function is_type(state, arguments, level, etype)
  return function(value) return type(value) == etype end
end
