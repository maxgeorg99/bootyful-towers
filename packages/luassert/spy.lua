  ...
  new = function(callback)
    ...
    if not util.callable(callback) then
      error(
        "Cannot spy on type '"
          .. type(callback)
          .. "', only on functions or callable elements",
        util.errorlevel()
      )
    end
      ...
      called = function(self, times, compare)
        if times or compare then
          local compare = compare
            or function(count, expected) return count == expected end
...
local function returned_with(state, arguments, level)
  ...
  if payload and payload.returned_with then
    local assertion_holds, matching_or_last_returnvallist =
      state.payload:returned_with(arguments)
...
local function called_with(state, arguments, level)
  ...
  if payload and payload.called_with then
    local assertion_holds, matching_or_last_arglist =
      state.payload:called_with(arguments)
...
local function called_at_least(state, arguments, level)
  ...
  return called(
    ...
    arguments,
    level,
    function(count, expected) return count >= expected end
  )
end
...
return setmetatable(spy, {
  __call = function(self, ...) return spy.new(...) end,
})
