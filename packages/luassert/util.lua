...
function util.matchargs(invocations_list, specs)
  ...
  assert(
    not util.is_arglist(invocations_list),
    "expected a list of arglist-object, got an arglist"
  )
...
function util.extract_keys(nspace, tokens)
  ...
  if key then
    error(
      "luassert: unknown modifier/" .. nspace .. ": '" .. key .. "'",
      util.errorlevel(2)
    )
  end
...
function util.is_arglist(object) return getmetatable(object) == arglist_mt end
