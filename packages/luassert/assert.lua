...
local __state_meta = {
  ...
  __call = function(self, ...)
    ...
    if assertion then
      ...
      local val, retargs =
        assertion.callback(self, arguments, util.errorlevel())
      ...
      if (not val) == self.mod then
        ...
        local err =
          geterror(message, rawget(self, "failure_message"), arguments)
...
obj = {
  ...
  add_formatter = function(self, callback) astate.add_formatter(callback) end,

  -- unregisters a formatter
  remove_formatter = function(self, fmtr) astate.remove_formatter(fmtr) end,
  ...
  set_parameter = function(self, name, value) astate.set_parameter(name, value) end,

  get_parameter = function(self, name) return astate.get_parameter(name) end,

  add_spy = function(self, spy) astate.add_spy(spy) end,

  snapshot = function(self) return astate.snapshot() end,
...
local __meta = {
  ...
  __index = function(self, key) return rawget(self, key) or self.state()[key] end,
}
